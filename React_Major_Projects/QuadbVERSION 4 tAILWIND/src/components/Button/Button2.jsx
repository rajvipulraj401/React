...
const Button2 = ({ name, width }) => {
  const [isHovered, setIsHovered] = useState(false);

  const responsiveWidth = width ? `sm:w-[${width}]` : "sm:w-[180px]";

  return (
    <button
      className={`
        ${responsiveWidth} sm:h-12 w-[140px] h-11 flex items-center justify-center text-white font-medium
        font-proxima-nova sm:text-[18px] text-[12px] leading-6
        relative overflow-hidden
        sm:hover:text-[19px] hover:font-bold
      `}
      style={{
